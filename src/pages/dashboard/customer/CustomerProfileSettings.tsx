...
const CustomerProfileSettings = ({ initialData }: Props) => {
  ...
  const formValues = useMemo(() => {
    return {
      ...
      // 3. ADD imageUrl to form state
      imageUrl: initialData?.imageUrl || "",
    };
  }, [initialData]);

  // 4. Extract methods to pass into FormProvider
  ...
  const onSubmit = async (data: CustomerProfileType) => {
    ...
    // 1. If the user selected a new photo, upload it first
      ...
      try {
        if (data.imageUrl instanceof File) {
          try {
            ...
            console.log(error);
            return; // Stop form submission if the image fails to upload
          }
        } else if (typeof data.imageUrl === "string") {
          // 2. If it's a string, they kept their existing photo
    ...
    const payload = {
      ...
      imageUrl: finalImageUrl, // This is now strictly a string
    };
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-12">
          <div className="md:col-span-1">
            ...
            <p className="text-sm text-muted">
              Pin your primary location so providers can easily find your
              address.
            </p>
          </div>
